...
import { Globe, Lock, Layers, Zap, CheckCircle } from "lucide-react";
    ...
    <div className="relative">
      {steps.map((step, i) => (
        <div key={step.title} className="relative pl-12 pb-10 last:pb-0">
          {i < steps.length - 1 && (
            <div className="absolute left-[11px] top-5 bottom-0 w-[2px] bg-gradient-to-b from-accent/30 to-violet/20" />
          )}
          <div
            className="absolute left-0 top-1 w-6 h-6 rounded-full border-2 flex items-center justify-center"
            style={{ borderColor: step.color, backgroundColor: "rgba(3,5,10,0.9)" }}
          >
