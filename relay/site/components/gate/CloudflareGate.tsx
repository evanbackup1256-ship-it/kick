...
import { mountTurnstileWidget, verifyGateToken } from "@/lib/turnstile";
...
export function CloudflareGate({ children }: { children: React.ReactNode }) {
  ...
  const gateRequiredRef = useRef<boolean | null>(null);
  ...
  useEffect(() => {
    ...
    const boot = async () => {
      ...
      gateRequiredRef.current = Boolean(gateCfg.required && gateCfg.serverVerify);
      if (!gateRequiredRef.current) {
        finish();
        return;
      }

      const siteKey = String(gateCfg.siteKey || "").trim();
      if (!siteKey) {
        finish();
        return;
      }
