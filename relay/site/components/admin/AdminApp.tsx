    ...
    <div className="admin-glass p-4">
      ...
      <div className="mt-5 flex h-28 items-end gap-1.5">
        {(data.length ? data : [0, 0, 0, 0, 0, 0, 0, 0]).slice(-28).map((value, index) => (
          <span
            key={`${label}-${index}`}
            className="min-w-1 flex-1 rounded-t bg-gradient-to-t from-accent/35 to-cyan/90 shadow-[0_0_18px_rgba(0,232,255,0.16)]"
...
function normalizeSeries(series?: SeriesMap): { key: string; values: number[] }[] {
  ...
  return Object.entries(series).map(([key, raw]) => ({
    ...
    values: Array.isArray(raw)
      ? raw.map((item) => (typeof item === "number" ? item : Number(item.value ?? item.count ?? 0)))
      : [],
  }));
}
...
function AdminLogin({ onLogin }: { onLogin: (token: string) => void }) {
  ...
  const login = useCallback(async () => {
    if (!key.trim()) {
      toast.error("Enter ADMIN_API_KEY");
      return;
    }
    setBusy(true);
    try {
  ...
  return (
    <div className="grid min-h-dvh place-items-center bg-bg-0 px-4 py-16 text-text">
      ...
      <div className="admin-login-card relative z-10 w-full max-w-xl p-6 md:p-8">
        ...
        <p className="site-kicker mt-8">Owner access</p>
        <h1 className="mt-4 text-4xl font-semibold tracking-tight md:text-5xl">Private telemetry deck.</h1>
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 py-6 md:px-6">
        <header className="admin-shell-head">
          <div>
            ...
            <h1 className="mt-3 text-4xl font-semibold tracking-tight md:text-6xl">Telemetry command center</h1>
          </div>
          <div className="flex flex-wrap gap-2">
            <button className="site-button border-white/10 bg-white/[0.035] text-text" type="button" onClick={() => void refresh()}>
              <RefreshCw className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} />
              Refresh
            </button>
            <button className="site-button border-red/20 bg-red/10 text-red" type="button" onClick={onLogout}>
        ...
        {error ? (
          <div className="admin-glass mt-5 border-red/30 bg-red/10 p-4 font-mono text-sm text-red">{error}</div>
        ) : null}
        ...
        <section className="mt-6 grid gap-4 xl:grid-cols-[0.9fr_1.1fr]">
          <div className="admin-glass admin-panel-scroll p-4">
            <div className="mb-4 flex items-center justify-between">
              <div>
                <p className="site-kicker">Private feed</p>
                <h2 className="mt-2 text-2xl font-semibold">Recent telemetry</h2>
              </div>
            ...
            <div className="space-y-2">
              {(feed.length ? feed : [{ event: "Waiting", message: "No recent admin telemetry returned yet." }]).map((item, index) => (
                <article key={`${item.at || "feed"}-${index}`} className="admin-feed-item">
                  <div className="flex items-center justify-between gap-3">
                    <strong>{item.event || item.kind || "telemetry"}</strong>
                    <time className="font-mono text-[10px] text-muted">{item.at ? new Date(item.at).toLocaleTimeString() : "live"}</time>
                  </div>
                  ...
                  {item.details ? <pre className="mt-2 whitespace-pre-wrap rounded border border-white/10 bg-black/30 p-2 text-[11px] text-red/90">{item.details}</pre> : null}
                </article>
              ))}
            </div>
          </div>
          ...
          <div className="grid gap-4">
            ...
            <div className="admin-glass p-4">
              <div className="mb-4 flex items-center justify-between">
                <div>
                  <p className="site-kicker">Live health</p>
                  <h2 className="mt-2 text-2xl font-semibold">Operational status</h2>
                </div>
...
function AdminAppInner() {
  ...
  const logout = useCallback(() => {
    ...
    if (active) {
      void fetch(apiUrl("/api/admin/logout"), { method: "POST", headers: adminHeaders(active) }).catch(() => {});
    }
    sessionStorage.removeItem(TOKEN_KEY);
    sessionStorage.removeItem(TOKEN_EXP_KEY);
    localStorage.removeItem(TOKEN_KEY);
