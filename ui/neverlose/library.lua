...
local Library do 
 ...
 FadeSpeed = 0.22,
        ...
        Folders = {
            Directory = "Alleral",
            Configs = "Alleral/Configs",
            Assets = "Alleral/Assets",
        },
 ...
 Library.EnsureTooltip = function(self)
 if self._TooltipItems then
 ...
 Instances:Create("UICorner", { Parent = accent.Instance, CornerRadius = UDimNew(0, 6) })
 ...
 local items = self._TooltipItems
 ...
 text = tostring(text or "")
 if text == "" or not target or not target.Instance then
 return
 end
 target:OnHover(function()
 self:ShowTooltip(text, target.Instance)
 end)
 target:OnHoverLeave(function()
 self:HideTooltip()
 end)
 end

 Library.HookElementTooltip = function(self, handle)
 if type(handle) ~= "table" then
 return
 end
 local text = handle.Tooltip or handle.Desc or handle.Description
 if handle._TooltipTarget and text and text ~= "" then
 self:BindTooltip(handle._TooltipTarget, text)
 handle._ArrrelTipHooked = true
 end
 end

 Library.AttachElementMeta = function(self, element, rowItem, data)
 element = element or {}
 data = data or {}
 element._TooltipTarget = rowItem
 element.Desc = data.Desc or data.Tooltip or data.desc or data.tooltip
 element.Tooltip = element.Desc
 if element.Desc and element.Desc ~= "" and rowItem then
 self:BindTooltip(rowItem, element.Desc)
 end
 return element
 end
 ...
 function Window:Init()
	...
	if not Window._MenuBindConnected then
		Window._MenuBindConnected = true
		Library:Connect(UserInputService.InputBegan, function(Input)
...
Library.Watermark = function(self, Data)
 ...
 return Library:AttachElementMeta(Toggle, Items["Toggle"], Data)
 end
 ...
 return Library:AttachElementMeta(Button, Items["Button"], Data)
 end
 ...
 return Library:AttachElementMeta(Slider, Items["Slider"], Data)
 end
 ...
 return Library:AttachElementMeta(Dropdown, Items["RealDropdown"], Data)
 end

 Library.Sections.Space = function(self, Data)
 Data = type(Data) == "table" and Data or {}
 local height = Data.Size or Data.Height or Data.size or 10
 Instances:Create("Frame", {
 ...
 local marker = Items["Label"].Instance:FindFirstChild("SubMarker")
 if marker then
 Tween:Create(marker, TweenInfo.new(1, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {Position = UDim2New(1, -16, 1, -6)}, true)
 end
 end
 else 
 ...
 local marker = Items["Label"].Instance:FindFirstChild("SubMarker")
 if marker then
 Tween:Create(marker, TweenInfo.new(1, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {Position = UDim2New(1, 30, 1, -6)}, true)
 end
 end
 end
 end
 ...
 Name = Data.Name or Data.name or Data.Title or Data.title or "",
 Flag = Data.Flag or Data.flag or Library:NextFlag(),
 ...
 local boxHeight = Textbox.Name ~= "" and 52 or 32
 local inputTop = Textbox.Name ~= "" and 18 or 0
 ...
 BackgroundTransparency = 1,
 Selectable = true,
 Size = UDim2New(1, 0, 0, boxHeight),
 Position = UDim2New(0, 0, 0, inputTop),
 ZIndex = 2,
 BorderSizePixel = 0,
 BackgroundColor3 = FromRGB(27, 26, 29)
 }) 
 ...
 return Library:AttachElementMeta(Textbox, Items["Textbox"], Data)
 end
